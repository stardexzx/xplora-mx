...
async function fetchRatings(): Promise<Record<string, number>> {
  const { data } = await supabase.from("reviews").select("negocio_id, rating");
...
export default function Home() {
  ...
  const [routeRequest, setRouteRequest] = useState<
    import("../component/Map").RouteRequest | null
  >(null);
  const [routeResult, setRouteResult] = useState<
    import("../app/api/route/route").RouteResult | null
  >(null);
  ...
  if (!ready)
    return (
      <div className="cp-loading">
        ...
        <p className="cp-loading__title">LocalIA</p>
...
style={{ height: "34px", objectFit: "contain", maxWidth: "110px" }
                ...
                >
                  {LANGUAGES.map((l) => (
                    <button
                      ...
                      style={{
                        color:
                          l.code === lang ? "var(--light-blue)" : undefined,
                      }}
                      onClick={() => {
                        setLang(l.code);
          ...
          <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
            ...
            <button
              className={`cp-voice${listening ? " on" : ""}`}
              onClick={handleVoice}
            >
              <svg
                width="15"
          ...
          {isMobile && (
            <div className="cp-tabs">
              {(["list", "map"] as const).map((v) => (
                <button
                  key={v}
                  className={`cp-tab${mobileView === v ? " active" : ""}`}
        ...
        {isMobile && (
          <nav className="cp-nav">
            {(
              ...
              <button
                key={nav.key}
                className={`cp-nbtn${activeNav === nav.key ? " active" : ""}`}
                onClick={nav.fn}
              >
                {nav.icon}
                {nav.label}
