...
const LANGUAGES = [
  { code: "es", flag: "MX" }, { code: "en", flag: "EN" },
  { code: "pt", flag: "PT" }, { code: "fr", flag: "FR" },
  { code: "de", flag: "DE" }, { code: "ja", flag: "JA" },
  { code: "ko", flag: "KO" }, { code: "ar", flag: "AR" },
];
...
function PriceBadge({ tags }: { tags?: string }) {
  ...
  if (t.includes("económico") || t.includes("barato") || t.includes("economico"))
    return <span style={{ display:"inline-flex", alignItems:"center", padding:"3px 10px", borderRadius:"10px", background:"rgba(16,185,129,0.15)", color:"#10b981", fontSize:"0.72rem", fontWeight:700 }}>$ Económico</span>;
  if (t.includes("moderado"))
    return <span style={{ display:"inline-flex", alignItems:"center", padding:"3px 10px", borderRadius:"10px", background:"rgba(245,158,11,0.15)", color:"#f59e0b", fontSize:"0.72rem", fontWeight:700 }}>$$ Moderado</span>;
  if (t.includes("premium") || t.includes("lujoso"))
    return <span style={{ display:"inline-flex", alignItems:"center", padding:"3px 10px", borderRadius:"10px", background:"rgba(239,68,68,0.15)", color:"#ef4444", fontSize:"0.72rem", fontWeight:700 }}>$$$ Premium</span>;
  return null;
}

const CAT_EMOJI: Record<string, string> = {
  comida:"🌮", tours:"🧭", hospedaje:"🏨", artesanias:"🏺", entretenimiento:"🎭",
};
...
export default function Home() {
  ...
  useEffect(() => {
    supabase.from("negocios").select("*, negocio_images(url, order_index)").eq("status","approved")
      .then(({ data, error }) => {
        if (!error && data) {
          ...
          setTodos(enriched); setNegocios(enriched);
        }
      });
  }, []);
  ...
  useEffect(() => {
    ...
    fetch("/api/translateUI", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ texts, lang }) })
  ...
  const applyCategory = (all: Negocio[], cat: string) =>
    cat === "all" ? all : all.filter(n => n.category?.toLowerCase().includes(cat === "artesanias" ? "artesa" : cat));

  const handleCategoryChange = (cat: string) => {
    setActiveCategory(cat); setNegocios(applyCategory(todosLosNegocios, cat)); setSelected(null);
  };

  const handleGetLocation = async () => {
    setLocationError("");
    try {
      const loc = await getUserLocation(); setUserLocation(loc);
      ...
      setNegocios(cercanos); setNearbyOnly(true);
    } catch { setLocationError("No se pudo obtener la ubicación."); }
  };

  const handleShowAll = () => { setNegocios(applyCategory(todosLosNegocios, activeCategory)); setNearbyOnly(false); setUserLocation(null); };
  ...
  const searchByText = async (text: string) => {
    ...
    try {
      ...
      if (filters.category?.trim()) qb = qb.ilike("category",`%${filters.category}%`);
      if (filters.rating > 0) qb = qb.gte("rating", filters.rating);
      if (Array.isArray(filters.tags)) filters.tags.filter(Boolean).forEach((tag:string) => { qb = qb.ilike("tags",`%${tag}%`); });
      const { data } = await qb;
      let resultados = (data ?? []).map((n:any) => ({
        ...n, images: (n.negocio_images ?? []).sort((a:any,b:any) => a.order_index - b.order_index).map((i:any) => i.url),
      })) as Negocio[];
      if (userLocation) {
        resultados = resultados.filter(n => n.lat && n.lng)
          .map(n => ({ ...n, distancia_km: getDistanceKm(userLocation,{ lat:n.lat, lng:n.lng }) }))
          .filter(n => n.distancia_km! <= RADIO_KM).sort((a,b) => a.distancia_km! - b.distancia_km!);
      }
      setNegocios(resultados); setActiveCategory("all"); setSelected(null);
  ...
  if (!ready) return (
    <div style={{ height:"100vh", display:"flex", alignItems:"center", justifyContent:"center", flexDirection:"column", gap:"12px", background:"#0a1412" }}>
      <div style={{ width:"44px", height:"44px", borderRadius:"12px", background:"#1d8a8c", display:"flex", alignItems:"center", justifyContent:"center" }}>
      ...
      <p style={{ fontWeight:800, fontSize:"1rem", color:"#fff", fontFamily:"'DM Sans', sans-serif" }}>XploraMX</p>
      <p style={{ color:"#5a8a84", fontSize:"0.82rem", fontFamily:"'DM Sans', sans-serif" }}>{t.translating}</p>
    </div>
  );
  ...
  return (
    <>
      ...
      <div className="xp" style={{ height:"100vh", display:"flex", flexDirection:"column", overflow:"hidden" }}>

        {/* ══ HEADER ══ */}
        <header style={{
          background:"var(--surface)", borderBottom:"1px solid var(--border)",
          padding: isMobile ? "10px 14px" : "12px 20px",
          display:"flex", flexDirection:"column", gap:"10px", flexShrink:0, zIndex:100,
        }}>
          {/* Logo row */}
          <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
            <div style={{ display:"flex", alignItems:"center", gap:"8px", flex:1 }}>
              <div style={{ width:"32px", height:"32px", borderRadius:"9px", background:"var(--teal)", display:"flex", alignItems:"center", justifyContent:"center" }}>
              ...
              <span style={{ fontWeight:800, fontSize:"1.05rem", color:"var(--text)", letterSpacing:"-0.01em" }}>
                Xplora
                <span style={{ background:"var(--orange)", color:"#fff", padding:"1px 7px 2px", borderRadius:"6px", fontSize:"0.72rem", fontWeight:700, marginLeft:"5px" }}>MX</span>
              </span>
            </div>

            {/* Nearby */}
            <button className={`xp-ibtn${nearbyOnly ? " on" : ""}`} onClick={nearbyOnly ? handleShowAll : handleGetLocation} title="Cerca de mí">
            ...
            <div style={{ position:"relative" }}>
              <button className="xp-ibtn" onClick={() => { setShowLangMenu(!showLangMenu); setShowUserMenu(false); }}>
                {LANGUAGES.find(l => l.code === lang)?.flag ?? "ES"}
              </button>
              {showLangMenu && (
                <div className="xp-drop" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"2px", minWidth:"120px" }}>
                  {LANGUAGES.map(l => (
                    <button key={l.code} className="xp-dbtn center"
                      style={{ color: l.code === lang ? "var(--teal-lt)" : undefined }}
            ...
            {user ? (
              <div style={{ position:"relative" }}>
                <button className="xp-ibtn" onClick={() => { setShowUserMenu(!showUserMenu); setShowLangMenu(false); }}>
                ...
                {showUserMenu && (
                  <div className="xp-drop">
                    {[
                      ...
                      <button key={item.href} className="xp-dbtn" onClick={() => { window.location.href = item.href; setShowUserMenu(false); }}>{item.label}</button>
                    ))}
                    <div style={{ height:"1px", background:"var(--border)", margin:"4px 0" }} />
                    <button className="xp-dbtn danger" onClick={async () => { await supabase.auth.signOut(); location.reload(); }}>{t.logout}</button>
                  </div>
                )}
              </div>
            ) : (
              <button className="xp-ibtn" style={{ width:"auto", padding:"0 12px", borderRadius:"18px" }} onClick={() => window.location.href = "/login"}>
                {t.login}
              </button>
            )}
          </div>
          ...
          <div style={{ display:"flex", gap:"8px", alignItems:"center" }}>
            <div style={{ position:"relative", flex:1 }}>
              ...
              <input className="xp-search" value={query}
                onChange={e => setQuery(e.target.value)}
                onKeyDown={e => e.key === "Enter" && handleSearch()}
                placeholder={t.searchPlaceholder}
              />
            </div>
            <button className={`xp-voice${listening ? " on" : ""}`} onClick={handleVoice}>
        ...
        {/* ══ CATEGORY BAR ══ */}
        <div style={{ display:"flex", gap:"8px", padding:"10px 14px", borderBottom:"1px solid var(--border)", background:"var(--bg)", overflowX:"auto", flexShrink:0, scrollbarWidth:"none" }}>
          <button className={`xp-pill${activeCategory === "all" ? " active" : ""}`} onClick={() => handleCategoryChange("all")}>
            🗺️ {categoryLabels["all"] ?? "Todos"}
          </button>
          {CATEGORIES.map(c => (
            <button key={c.key} className={`xp-pill${activeCategory === c.key ? " active" : ""}`} onClick={() => handleCategoryChange(c.key)}>
        ...
        <div style={{ display:"flex", flex:1, overflow:"hidden", flexDirection:"column" }}>
            ...
            <div style={{ display:"flex", background:"var(--surface)", borderBottom:"1px solid var(--border)", flexShrink:0 }}>
              {(["list","map"] as const).map(v => (
                <button key={v} onClick={() => setMobileView(v)} style={{
                  ...
                  fontFamily:"'DM Sans',sans-serif", fontWeight: mobileView === v ? 700 : 500, fontSize:"0.82rem",
                  ...
                  {v === "list"
                    ? <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg>Lista {negocios.length > 0 && `(${negocios.length})`}</>
                    : <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>Mapa</>
                  }
                </button>
              ))}
            </div>
          ...
          <div style={{ display:"flex", flex:1, overflow:"hidden" }}>
            ...
            <div style={{
              ...
              {nearbyOnly && (
                <div style={{ margin:"0 12px 10px", padding:"8px 14px", borderRadius:"10px", background:"var(--teal-a)", border:"1px solid var(--border)", fontSize:"0.78rem", color:"var(--text-soft)", fontWeight:500 }}>
                  {interpolate(t.nearbyCount, { n: negocios.length, km: RADIO_KM })}
                </div>
              )}

              {negocios.length === 0 ? (
                <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:"10px", padding:"3rem 1rem", color:"var(--muted)" }}>
                  <span style={{ fontSize:"2.5rem" }}>🔍</span>
                  <p style={{ fontSize:"0.9rem", fontWeight:600, color:"var(--text-soft)" }}>Sin resultados</p>
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      <p style={{ margin:"0 0 2px", fontWeight:700, fontSize:"0.92rem", color:"var(--text)", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{n.name}</p>
                      <p style={{ margin:"0 0 7px", fontSize:"0.78rem", color:"var(--muted)", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
                    ...
                    <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:"5px", flexShrink:0 }}>
                      {n.rating != null && (
                        <div style={{ display:"flex", alignItems:"center", gap:"3px" }}>
                          ...
                          <span style={{ fontSize:"0.8rem", fontWeight:700, color:"var(--text-soft)" }}>{n.rating.toFixed(1)}</span>
                        </div>
                      )}
            ...
            <div style={{ flex:1, position:"relative", display: isMobile && mobileView !== "map" ? "none" : "block" }}>
              <BusinessMap
                negocios={negocios} selectedId={selectedNegocio?.id ?? null}
                userLocation={userLocation} onSelectNegocio={(n) => setSelected(n)}
              />
              {selectedNegocio && <BusinessPopup negocio={selectedNegocio} onClose={() => setSelected(null)} />}
              <button className="xp-qr" onClick={() => window.location.href = "/qr"}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="4" height="4"/></svg>
                QR
              </button>
              {user && (
                <button className="xp-fab" onClick={() => window.location.href = "/dashboard"}>+ Registrar negocio</button>
              )}
            </div>
          </div>
        </div>
        ...
        {isMobile && (
          <nav className="xp-nav">
            {([
              { key:"mapa",      label:"Mapa",      icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>,      fn:() => { setActiveNav("mapa"); setMobileView("map"); } },
              { key:"asistente", label:"Asistente", icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2M12 19v4M8 23h8"/></svg>, fn:() => { setActiveNav("asistente"); handleVoice(); } },
              { key:"negocio",   label:"Negocio",   icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>, fn:() => { setActiveNav("negocio"); window.location.href = user ? "/mis-negocios" : "/login"; } },
              { key:"perfil",    label:"Perfil",    icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>, fn:() => { setActiveNav("perfil"); window.location.href = user ? "/perfil" : "/login"; } },
            ] as const).map(nav => (
              <button key={nav.key} className={`xp-nbtn${activeNav === nav.key ? " active" : ""}`} onClick={nav.fn}>
                {nav.icon}{nav.label}
              </button>
            ))}
          </nav>
