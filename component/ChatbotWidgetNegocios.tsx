...
const MODULES = [
  {
    key: "diagnostico",
    label: "Diagnóstico",
    sublabel: "Analiza tu negocio",
    color: "#1CA8F7",
    bg: "rgba(28,168,247,0.15)",
    icon: (
      <svg
        viewBox="0 0 24 24"
        fill="none"
  ...
  },
  {
    key: "finanzas",
    label: "Finanzas",
    sublabel: "Flujo y costos",
    color: "#5b8aff",
    bg: "rgba(91,138,255,0.15)",
    icon: (
      <svg
        viewBox="0 0 24 24"
        fill="none"
      ...
      >
        ...
        <path d="M12 7v10M9 9.5C9 8.12 10.34 7 12 7s3 1.12 3 2.5c0 2.5-3 3-3 5" />
      </svg>
    ),
  },
  {
    key: "turistas",
    label: "Captar turistas",
    sublabel: "Atrae visitantes",
    color: "#34d399",
...
const BOT_RESPONSES: Record<string, string> = {
  diagnostico:
    "Vamos a analizar el estado de tu negocio. ¿Cuál es el principal reto que enfrentas hoy?",
  coppel:
    "Módulo Coppel activo. ¿En qué te puedo apoyar con tus productos o financiamiento?",
  finanzas:
    "Puedo ayudarte con flujo de efectivo, costos y proyecciones. ¿Por dónde empezamos?",
  turistas:
    "¡Excelente! Te ayudo a atraer más visitantes. ¿Tienes presencia en redes sociales?",
    ...
    >
      {MODULES.map((mod) => (
        ...
        >
          ...
          {mod.status ? (
            <span style={{ fontSize: 10, fontWeight: 700, color: "#F0D224" }}>
              {mod.status}
            </span>
          ) : (
...
export default function ChatbotNegocios() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 0,
      role: "bot",
      text: "Hola 👋 ¿En qué módulo te puedo ayudar hoy?",
      showModules: true,
    },
  ]);
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      setIsTyping(false);
      addMsg("bot", BOT_RESPONSES.default);
    }, 1000);
  };
          ...
          style={{
            ...
            boxShadow:
              !inputValue.trim() || isTyping
                ? "none"
                : "0 4px 12px rgba(28,66,232,0.35)",
          }}
        >
          <svg
            viewBox="0 0 24 24"
