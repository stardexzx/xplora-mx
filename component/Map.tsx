...
export default function BusinessMap({ negocios, selectedId, userLocation, onSelectNegocio, routeRequest, onRouteResult }: MapProps) {
  ...
  useEffect(() => {
    ...
    if (!routeRequest) { onRouteResult?.(null); return; }

    fetch("/api/route", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ origin: routeRequest.origin, destination: routeRequest.destination, mode: routeRequest.mode }),
    })
      .then(r => r.json())
      ...
      .catch(err => { console.error("Error calculando ruta:", err); onRouteResult?.(null); });
  }, [routeRequest, isLoaded]);

  // Cerrar popup al hacer click en el mapa
  const handleMapClick = useCallback(() => { onSelectNegocio(null); }, [onSelectNegocio]);
